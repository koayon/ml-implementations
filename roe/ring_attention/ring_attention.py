...
from .distributed import AllGather, split_by_rank
from .ring import all_ring_pass, get_rank, get_world_size, is_distributed
...
class RingRotaryEmbedding(Module):
    ...
    def forward(self, seq_len: int, offset=0):
        ...
        pos = pos.type_as(self.inv_freq)  # type: ignore
...
@autocast(enabled=False)
def apply_rotary_pos_emb(pos: t.Tensor, theta):
    pos = rearrange("n d -> n 1 d", pos)
    return theta * pos.cos() + rotate_half(theta) * pos.sin()
...
def sharded_batch_to_sharded_seq(x: Tensor, mask: Optional[Tensor], seq_size: int):
    ...
    if exists(mask):
        mask = rearrange("(b s) n -> b (s n)", mask, s=num_sharded_batches)  # type: ignore
        mask = mask.split(seq_size, dim=-1)  # type: ignore
        mask, _ = split_by_rank(mask)

    return (x, mask), sizes, num_sharded_batches


def sharded_seq_to_sharded_batch(logits: t.Tensor, sizes, num_sharded_batches=1):
    all_gather = AllGather(dim=-2)  # all gather across sequence

    logits, _ = all_gather(logits)

    logits = rearrange("b (s n) c -> (b s) n c", logits, s=num_sharded_batches)  # type: ignore
...
class RingAttention(Module):
    ...
    def forward(
        ...
        any_cuda_inputs = any([matr.is_cuda for matr in (q, k, v)])
        ...
        if self.force_regular_attn:
            ...
            from .ring_flash_attention_cuda import ring_flash_attention_cuda
...
class RingTransformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        max_lookback_seq_len = cast_tuple(max_lookback_seq_len, depth)  # type: ignore
        assert len(max_lookback_seq_len) == depth  # type: ignore

        for layer_max_lookback_seq_len in max_lookback_seq_len:  # type: ignore
    ...
    def forward(
        ...
        if auto_shard_seq:
            ...
            if exists(labels):
                labels, label_mask = maybe_pad_seq_and_mask(
                    labels, mask[:, 1:], self.ring_seq_size  # type: ignore
                )
                labels.masked_fill_(~label_mask, self.ignore_index)  # type: ignore
            ...
            (x, mask), batch_sizes, num_sharded_batches = sharded_batch_to_sharded_seq(
                x, mask, self.ring_seq_size  # type: ignore
            )

            if exists(labels):
                (labels, _), *_ = sharded_batch_to_sharded_seq(
                    labels, None, self.ring_seq_size  # type: ignore
                )
        ...
        if return_loss:
            ...
            ce_loss = F.cross_entropy(logits, labels, ignore_index=self.ignore_index)  # type: ignore
        ...
        return logits[:, :seq_len]  # type: ignore
