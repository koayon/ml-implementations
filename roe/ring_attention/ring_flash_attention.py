...
class RingFlashAttentionFunction(Function):
    ...
    def forward(
        ...
        if exists(max_lookback_seq_len):
            ...
            max_ring_passes = math.ceil(max_lookback_seq_len / per_machine_seq_size)  # type: ignore
            num_lookback_buckets = max_lookback_seq_len // bucket_size  # type: ignore
        ...
        row_ring_rank = (get_rank() % ring_size) if ring_reduce_col else 0  # type: ignore
                ...
                for ind, (qc, oc, row_sums, row_maxes) in enumerate(row_splits):
                    ...
                    if causal:
                        ...
                        if striped_ring_attn:
                            ...
                            attn_weights.masked_fill_(causal_mask, max_neg_value)  # type: ignore
                            ...
                            if row_bucket_index == col_bucket_index:
                                ...
                                attn_weights.fill_(max_neg_value)  # type: ignore

                    block_row_maxes = attn_weights.amax(dim=-1, keepdims=True)  # type: ignore
                    ...
                    block_row_sums = exp_weights.sum(dim=-1, keepdims=True).clamp(  # type: ignore
                        min=EPSILON
                    )
        ...
        o.div_(rearrange("b h n 1 -> b n h 1", all_row_sums))  # type: ignore
    ...
    def backward(ctx, do):
        ...
        for (ring_rank, is_last), (  # type: ignore
            (kv_and_dkv, mask),
